...
import com.finalproject.HRM.repositories.LeaveRequestRepository;
...
public Page<LeaveRequestDto> getAllLeaveRequests(int index, int size) {
	Pageable page=PageRequest.of(index, size);
		...
		for(LeaveRequest l: leaveRequests) {
			...
			LeaveRequestDto leaveRequest=LeaveRequestDto.builder().id(l.getId())
						.fromDate(l.getFromDate())
						.toDate(l.getToDate())
						.leaveReason(l.getLeaveReason())
						.leave(leave)
		...
		Page<LeaveRequestDto> leaveRequestDtoPage= new PageImpl<LeaveRequestDto>(leaveRequestDtoList,page,leaveRequestDtoList.size());
...
public LeaveRequestDto updateLeaveRequestDto(String id, LeaveRequestDto leaveRequest) {
	User employee=User.builder().id(leaveRequest.getEmployee().getId()).build();
	Leave leave= Leave.builder().id(leaveRequest.getLeave().getId()).build();
	LeaveRequest leaveRequestEntity=LeaveRequest.builder().id(id)
			.fromDate(leaveRequest.getFromDate())
			.toDate(leaveRequest.getToDate())
			.leaveReason(leaveRequest.getLeaveReason())
			.leave(leave)
